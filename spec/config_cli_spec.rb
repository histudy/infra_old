...
describe file(property['php_ini_dir'] + '/cli/php.ini') do
  ...
  if php_ini.key?('enable_post_data_reading')
    its(:content) { should match(/^enable_post_data_reading = #{to_ini_value(php_ini['enable_post_data_reading'])}$/) }
  end
  ...
  if php_ini.key?('auto_detect_line_endings')
    its(:content) { should match(/^auto_detect_line_endings = #{to_ini_value(php_ini['auto_detect_line_endings'])}$/) }
  end
  ...
  if php_ini['session'].key?('upload_progress')
    ...
    if ini_up_progress.key?('enabled')
      its(:content) { should match(/^session.upload_progress.enabled = #{to_ini_value(ini_up_progress['enabled'])}$/) }
    end
    if ini_up_progress.key?('cleanup')
      its(:content) { should match(/^session.upload_progress.cleanup = #{to_ini_value(ini_up_progress['cleanup'])}$/) }
    end
    if ini_up_progress.key?('prefix')
      its(:content) { should match(/^session.upload_progress.prefix = #{to_ini_value(ini_up_progress['prefix'])}$/) }
    end
  ...
  if php_ini.key?('exif')
    ...
    if ini_exif.key?('decode_unicode_intel')
      its(:content) { should match(/^exif.decode_unicode_intel = #{to_ini_value(ini_exif['decode_unicode_intel'])}$/) }
    end
  ...
  if php_ini.key?('mbstring')
    ...
    if ini_mbstring.key?('strict_detection')
      its(:content) { should match(/^mbstring.strict_detection = #{to_ini_value(ini_mbstring['strict_detection'])}$/) }
    end
  ...
  if php_ini.key?('oci8')
    ...
    if ini_oci8.key?('statement_cache_size')
      its(:content) { should match(/^oci8.statement_cache_size = #{to_ini_value(ini_oci8['statement_cache_size'])}$/) }
    end
